...
# Frozen, rounded thresholds from the prior exploratory quartile scan.
ATR_MIN = 2.8
DD20_MAX = -5.5
TURNOVER20_MIN = 1_000_000_000.0
...
def is_kuitto(g, i):
    ...
    r = g.iloc[i]
    req = ["MA5","MA25","O","C","Vo"]
    if any(pd.isna(r[x]) for x in req) or r["O"] <= 0:
        return False
    ...
    close_vs_ma5 = (r["C"] / r["MA5"] - 1) * 100
    if close_vs_ma5 > CLOSE_VS_MA5_MAX_PCT:
        return False
...
def summarize(part, split):
    return {
        "overall_10y": metrics(part),
        "older_5y": metrics(part[part["signal_date_dt"] < split]),
        "recent_5y": metrics(part[part["signal_date_dt"] >= split]),
        "yearly": {
            str(int(y)): metrics(p)
...
def main():
    ...
    variants = {
        ...
        "atr_only": t["atr14_pct"] >= ATR_MIN,
        "atr_plus_dd20": (t["atr14_pct"] >= ATR_MIN) & (t["dd20_pct"] <= DD20_MAX),
        "atr_plus_turnover": (t["atr14_pct"] >= ATR_MIN) & (t["avg_turnover20"] >= TURNOVER20_MIN),
    }

    summary = {
        "strategy": "Frozen Kuitto signal; next-open entry; fixed 10-session exit",
        "thresholds": {
            "atr14_pct_min": ATR_MIN,
            "dd20_pct_max": DD20_MAX,
            "avg_turnover20_min_yen": TURNOVER20_MIN,
        },
        "variants": {},
        "note": "These are exploratory filters chosen from the previous 10-year feature scan. Split results test regime robustness, but are not clean out-of-sample because thresholds were selected after viewing the archive.",
    }

    tagged = t.copy()
    for name, mask in variants.items():
        part = t[mask.fillna(False)].copy()
        summary["variants"][name] = summarize(part, split)
        tagged[name] = mask.fillna(False)

    RESULT_DIR.mkdir(parents=True, exist_ok=True)
    tagged.drop(columns=["signal_date_dt"]).to_csv(
        RESULT_DIR / "kuitto_runner_3way_trades.csv", index=False
    )
    (RESULT_DIR / "kuitto_runner_3way_summary.json").write_text(
