...
#
# 90日だと、screening.htmlが表示するMA75（75日移動平均）が計算できるのは
# 直近の(90-75)=15日分しかなく、「3か月」表示(直近66日)の範囲ではMA75線が
# ほとんど描画されない状態になっていた。MA75の計算に必要な75日分の助走に加えて
# 「3か月」表示分（66日）が全区間MA75付きで見えるよう、75+66=141日以上を確保する。
CHART_BUSINESS_DAYS = 200

# 上記を集める際に遡ってよい最大日数（休日・データ欠損時のセーフティ）。
# 200営業日だと土日・祝日を考えると実質280日前後になるため、余裕を持たせる。
CHART_MAX_LOOKBACK_DAYS = 340
